...
def test_parse_literal_field():
    ...
    assert _parse_literal_field("prefix_status", {"prefix_status": "COMPLETED"}, field_info) == "COMPLETED"
    
    # Optional literal field with value
    opt_field_info = FieldInfo(annotation=Optional[Literal["PENDING", "COMPLETED"]])
    assert _parse_literal_field("prefix_opt_status", {"prefix_opt_status": "PENDING"}, opt_field_info) == "PENDING"
    
    # Optional literal field with empty string
    assert _parse_literal_field("prefix_opt_status", {"prefix_opt_status": ""}, opt_field_info) is None
    
    # Optional literal field with "-- None --" value (UI representation of None)
    assert _parse_literal_field("prefix_opt_status", {"prefix_opt_status": "-- None --"}, opt_field_info) is None
...
def test_parse_simple_field():
    ...
    # Basic field
    assert _parse_simple_field("prefix_name", {"prefix_name": "Test Name"}) == "Test Name"
    
    # Optional field with value
    opt_field_info = FieldInfo(annotation=Optional[str])
    assert _parse_simple_field("prefix_desc", {"prefix_desc": "Test Description"}, opt_field_info) == "Test Description"
    
    # Optional field with empty string
    assert _parse_simple_field("prefix_desc", {"prefix_desc": ""}, opt_field_info) is None
...
def test_parse_nested_model_field():
    ...
    # Parse nested model
    result = _parse_nested_model_field("nested", form_data, SimpleNested, field_info, parent_prefix="prefix_")
    ...
    result = _parse_nested_model_field("missing", {}, SimpleNested, field_info, parent_prefix="prefix_")
...
def test_parse_list_item_key():
    ...
    result = _parse_list_item_key("prefix_tags_new_12345", list_defs, base_prefix="prefix_")
    assert result == ("tags", "new_12345", None, True)
    
    # Model list item
    result = _parse_list_item_key("prefix_addresses_0_sub_field", list_defs, base_prefix="prefix_")
    assert result == ("addresses", "0", "sub_field", False)
    
    # Model list item with "new_" prefix
    result = _parse_list_item_key("prefix_addresses_new_12345_sub_field", list_defs, base_prefix="prefix_")
...
def test_parse_non_list_fields():
    ...
    parsed = _parse_non_list_fields(form_data, ParserTestModel, list_field_defs, base_prefix="prefix_")
