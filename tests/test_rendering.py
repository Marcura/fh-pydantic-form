...
def test_render_globally_disabled_simple_form(globally_disabled_simple_client):
    ...
    assert (
        re.search(
            r'name="test_simple_globally_disabled_name"[^>]*disabled', response.text
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_simple_globally_disabled_age"[^>]*disabled', response.text
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_simple_globally_disabled_score"[^>]*disabled', response.text
        )
        is not None
    )
...
def test_render_partially_disabled_simple_form(partially_disabled_simple_client):
    ...
    assert (
        re.search(
            r'name="test_simple_partially_disabled_age"[^>]*disabled', response.text
        )
        is not None
    )
...
def test_render_globally_disabled_complex_form(globally_disabled_complex_client):
    ...
    assert (
        re.search(
            r'name="test_complex_globally_disabled_name"[^>]*disabled', response.text
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_globally_disabled_age"[^>]*disabled', response.text
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_globally_disabled_is_active"[^>]*disabled',
            response.text,
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_globally_disabled_creation_date"[^>]*disabled',
            response.text,
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_globally_disabled_start_time"[^>]*disabled',
            response.text,
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_globally_disabled_status"[^>]*disabled', response.text
        )
        is not None
    )
    ...
    assert (
        re.search(
            r'name="test_complex_globally_disabled_main_address_street"[^>]*disabled',
            response.text,
        )
        is not None
    )
    ...
    if "test_complex_globally_disabled_tags_0" in response.text:
        ...
        assert (
            re.search(
                r'name="test_complex_globally_disabled_tags_0"[^>]*disabled',
                response.text,
            )
            is not None
        )
    ...
    assert (
        re.search(
            r"<button[^>]*?(?=.*\buk-button-danger\b)(?=.*disabled)[^>]*>",
            response.text,
        )
        is not None
    )  # Delete button
    assert (
        re.search(
            r"<button[^>]*?(?=.*\buk-button-secondary\b)(?=.*disabled)[^>]*>",
            response.text,
        )
        is not None
    )  # Add button
    assert (
        re.search(
            r"<button[^>]*?(?=.*\bmove-up-btn\b)(?=.*disabled)[^>]*>", response.text
        )
        is not None
    )  # Move up button
    assert (
        re.search(
            r"<button[^>]*?(?=.*\bmove-down-btn\b)(?=.*disabled)[^>]*>", response.text
        )
        is not None
    )  # Move down button
...
def test_render_partially_disabled_complex_form(partially_disabled_complex_client):
    ...
    assert (
        re.search(
            r'name="test_complex_partially_disabled_name"[^>]*disabled', response.text
        )
        is not None
    )
    ...
    assert (
        re.search(
            r'name="test_complex_partially_disabled_main_address_street"[^>]*disabled',
            response.text,
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_partially_disabled_main_address_city"[^>]*disabled',
            response.text,
        )
        is not None
    )
    assert (
        re.search(
            r'name="test_complex_partially_disabled_main_address_is_billing"[^>]*disabled',
            response.text,
        )
        is not None
    )
    ...
    if "test_complex_partially_disabled_tags_0" in response.text:
        assert (
            re.search(
                r'name="test_complex_partially_disabled_tags_0"[^>]*disabled',
                response.text,
            )
            is not None
        )
    ...
    other_address_match = re.search(
        r'<input[^>]*name="test_complex_partially_disabled_other_addresses_0_street"[^>]*>',
        response.text,
    )
    ...
    custom_detail_match = re.search(
        r'<input[^>]*name="test_complex_partially_disabled_custom_detail_value"[^>]*>',
        response.text,
    )
