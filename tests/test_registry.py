...
def test_register_type_renderer():
    ...
    try:
        ...
        # Verify registration
        assert registry._type_renderers.get(SampleModelForRegistry) is CustomFieldRenderer
    finally:
        # Restore original renderers
        registry._type_renderers = original_renderers
...
def test_register_type_name_renderer():
    ...
    try:
        ...
        registry.register_type_name_renderer("SampleModelForRegistry", CustomFieldRenderer)
        
        # Verify registration
        assert registry._type_name_renderers["SampleModelForRegistry"] is CustomFieldRenderer
    finally:
        # Restore original renderers
        registry._type_name_renderers = original_renderers
...
def test_register_type_renderer_with_predicate():
    ...
    try:
        ...
        # Get initial count
        initial_count = len(registry._predicate_renderers)
        
        # Register a predicate renderer
        registry.register_type_renderer_with_predicate(is_sample_model, CustomFieldRenderer)
        
        # Verify registration - find our predicate in the list
        assert any(p[0] is is_sample_model and p[1] is CustomFieldRenderer
        ...
        # Restore original renderers
        registry._predicate_renderers = original_renderers
...
def test_register_list_item_renderer():
    ...
    try:
        ...
        registry.register_list_item_renderer(SampleModelForRegistry, CustomFieldRenderer)
        
        # Verify registration
        assert registry._list_item_renderers.get(SampleModelForRegistry) is CustomFieldRenderer
    finally:
        # Restore original renderers
        registry._list_item_renderers = original_renderers
...
def test_get_renderer_for_literal_type():
    ...
    assert registry.get_renderer("opt_status", opt_literal_field) is LiteralFieldRenderer
...
def test_get_renderer_for_basemodel_type():
    ...
    try:
        ...
        # Restore original renderers
        registry._type_renderers = original_renderers
        registry._predicate_renderers = original_predicates
...
def test_get_list_item_renderer():
    ...
    try:
        ...
        assert registry.get_list_item_renderer(SampleModelForRegistry) is CustomFieldRenderer
