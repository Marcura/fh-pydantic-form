...
```xml
<Plan>
Rewrite the `README.md` file to provide a comprehensive overview of the `fh-pydantic-form` library. This includes:
1.  A clear statement of the library's purpose.
2.  Installation instructions using both `pip` and `uv`.
3.  A clear, concise basic usage example demonstrating model definition, form rendering, and request validation within a FastHTML application.
4.  A summary of key features.
</Plan>

<file path="/Users/oege/projects/fh-pydantic-form/README.md" action="rewrite">
  <change>
    <description>Update README with installation instructions, purpose, example, and features.</description>
    <content>
===
# fh-pydantic-form
...
`fh-pydantic-form` simplifies creating web forms by automatically generating the necessary HTML input elements based on your Pydantic model definitions. It integrates seamlessly with [FastHTML](https://github.com/AnswerDotAI/fasthtml) and leverages [MonsterUI](https://github.com/AnswerDotAI/monsterui) components for styling (though it can be adapted for other UI libraries).
...
```bash
uv pip install fh-pydantic-form
```
...
Here's a simple example demonstrating how to create and validate a form:
...
class SimpleModel(BaseModel):
    """Model representing a simple form"""
    name: str = "Default Name"
    age: int = Field(42, description="Age of the customer") # description gets displayed as tooltip
    is_active: bool = True
...
# (Optional) Register routes for form reset, refresh and list adding/deleting:
